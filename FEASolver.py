...
grid_points = 20 # The number of nodes that exist along the axes of the boundary (inclusive)
equ_array_length = 2
...
X_Grid = [[num.average(elements_with_coordinates[xcounter][ycounter][0:2]) 
          for xcounter in range(number_of_divisions - 1) ] for ycounter in range(number_of_divisions - 1)]

Y_Grid = [[num.average(elements_with_coordinates[xcounter][ycounter][2:4]) 
          for xcounter in range(number_of_divisions - 1) ] for ycounter in range(number_of_divisions - 1)]

values = [[0 for xcounter in range(number_of_divisions - 1) ] for ycounter in range(number_of_divisions - 1)]

for counter_x in range(len(X_Grid)):
    for counter_y in range(len(Y_Grid[0])):
